...
@Component({
    selector: 'schedule-dialog-entry',
    template: ''
})
export class ScheduleDialogEntryComponent {
    constructor(public dialog: MatDialog, private route: ActivatedRoute, public dataService: SessionDataService,
        private zone: NgZone, private navigation: NavigationService, private router: Router) {
        this.openDialog();
    }
    ngOnInit() {
...
@Component({
    ...
    standalone: true,
    imports: [MatDialogModule, MatButtonModule, CommonModule, RouterModule, FontAwesomeModule, LinkTypeToIconPipe, MatIconModule, MatChipsModule],
...
class ScheduleDialogComponent {
    ...
    constructor(private dataService: SessionDataService, private speakerService: LoadSpeakersService, private service: LoadScheduleService) {
    }
    ngOnInit() {
